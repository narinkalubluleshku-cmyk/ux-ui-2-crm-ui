...
async function runHeroIntro() {
  ...
  // Начинаем сворачивать фразу ровно после появления последнего слова
  // («меня»), не дожидаясь следующего элемента последовательности — фото.
  ...
  // Фото улетает одновременно с началом исчезновения фразы.
  ...
  const updateFlightCurve = () => {
    ...
    // y = x³: сначала фото почти горизонтально огибает фразу,
    // затем заметнее поднимается к верхнему правому углу.
  ...
  // Быстрый пролёт до последних 10% траектории.
  ...
  // Финальные 10% пути — мягкое замедление без вылета за цель.
  timeline.to(flightCurve, {
    progress: 1,
    duration: 0.3,
    ease: "power2.out",
